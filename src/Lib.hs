...
comma deci com string
    | not (checkNumber deci string) = Nothing
    | deci `elem` string = Just $ (commaNoDots com $ fst split) ++ (deci : (snd split))
    | otherwise = Just $ commaNoDots com string        
    where
        split = splitAtFirst deci string :: (String, String)
